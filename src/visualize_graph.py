...
def main() -> None:
    ...
    structure_output = (
        Path(f"results/visuals/{repo_name}_graph_structure.png").resolve()
        if args.structure_output == "results/visuals/graph_structure.png"
        else Path(args.structure_output).resolve()
    )
    structure_output_imports = (
        Path(f"results/visuals/{repo_name}_graph_structure_imports.png").resolve()
        if args.structure_output_imports == "results/visuals/graph_structure_imports.png"
        else Path(args.structure_output_imports).resolve()
    )
    structure_output_in_file = (
        Path(f"results/visuals/{repo_name}_graph_structure_in_file.png").resolve()
        if args.structure_output_in_file == "results/visuals/graph_structure_in_file.png"
        else Path(args.structure_output_in_file).resolve()
    )
    analysis_output = (
        Path(f"results/visuals/{repo_name}_graph_degree_analysis.png").resolve()
        if args.analysis_output == "results/visuals/graph_degree_analysis.png"
        else Path(args.analysis_output).resolve()
    )
    analysis_output_imports = (
        Path(f"results/visuals/{repo_name}_graph_degree_analysis_imports.png").resolve()
        if args.analysis_output_imports == "results/visuals/graph_degree_analysis_imports.png"
        else Path(args.analysis_output_imports).resolve()
    )
    analysis_output_in_file = (
        Path(f"results/visuals/{repo_name}_graph_degree_analysis_in_file.png").resolve()
        if args.analysis_output_in_file == "results/visuals/graph_degree_analysis_in_file.png"
        else Path(args.analysis_output_in_file).resolve()
    )
    summary_output = (
        Path(f"results/reports/{repo_name}_visual_summary.txt").resolve()
        if args.summary_output == "results/reports/visual_summary.txt"
        else Path(args.summary_output).resolve()
    )
    ...
    if not args.skip_structure:
        ...
        print("Skipping structure visualization because --skip-structure was provided.")
    ...
    print(f"Overall structure visualization saved to: {structure_output}")
    print(f"IMPORTS structure visualization saved to: {structure_output_imports}")
    print(f"IN_FILE structure visualization saved to: {structure_output_in_file}")
    print(f"Overall degree analysis saved to: {analysis_output}")
    print(f"IMPORTS degree analysis saved to: {analysis_output_imports}")
    print(f"IN_FILE degree analysis saved to: {analysis_output_in_file}")
