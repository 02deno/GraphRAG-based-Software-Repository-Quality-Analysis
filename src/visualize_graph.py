...
def plot_structure_subgraph(
    ...
    plt.title("Graph Structure (Top-Degree Subgraph)")
    plt.axis("off")
...
def plot_degree_bars(
    ...
    """Plot top in-degree and out-degree files as bar charts."""
    ...
    axes[0].set_title(f"Top {top_k} Most Imported Files (In-Degree)")
    ...
    axes[1].set_title(f"Top {top_k} Most Importing Files (Out-Degree)")
...
def main() -> None:
    ...
    selected_nodes = top_nodes_by_total_degree(
        in_degree=in_degree,
    ...
    plot_structure_subgraph(nx_graph, selected_nodes, path_by_id, structure_output)
    plot_degree_bars(in_degree, out_degree, path_by_id, args.top_k, analysis_output)
