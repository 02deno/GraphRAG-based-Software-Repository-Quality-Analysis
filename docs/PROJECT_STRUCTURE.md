...
- **`config.py`**: Project root resolution, ``load_project_dotenv()`` (``.env`` with optional ``GRAPHRAG_DOTENV_OVERRIDE`` second pass), and Flask configuration (e.g. ``FLASK_SECRET_KEY``)
...
- **`analysis_llm_insights.py`**: Optional one-shot OpenAI-compatible call that turns metrics + ``analysis.txt`` + chart filenames into structured JSON (``graphrag_llm_insights.json``); tunable via ``GRAPHRAG_INSIGHTS_COMPACT``, ``GRAPHRAG_INSIGHTS_MAX_TOKENS``, ``GRAPHRAG_INSIGHTS_JSON_OBJECT``; used by ``web.analysis_results_llm_insights`` and surfaced on ``templates/results_final.html``. After a successful save (or on cache hit), ``refresh_llm_insights_source_chunks`` updates ``graphrag_source_chunks.jsonl`` so workspace chat can retrieve the narrative. ``format_llm_insights_for_source_index`` flattens the JSON for lexical indexing.
...
- **`source_context.py`**: Writes ``graphrag_run_meta.json``, builds ``graphrag_source_chunks.jsonl`` from ``File`` ``*.py`` paths, repo-wide ``*.md`` (and root ``.rst``/``.txt`` readmes), virtual ``_analysis/*`` chunks from pipeline metrics/text, and optional ``llm_insights`` virtual chunks from ``graphrag_llm_insights.json`` (``refresh_llm_insights_source_chunks`` merges them after the results-page insights API saves). Ranks all chunks lexically for chat. ``_analysis/*`` chunks get a **score bonus** when the question clearly targets graph metrics or cached AI interpretation phrases, and a **penalty** when the question is about the product/codebase (so host paths in reports do not drown real source files). ``included_chunks`` may include ``kind``, ``excerpt`` (for UI), path, line span, score.
...
- **`openai_compatible_client.py`**: OpenAI-compatible ``POST /chat/completions`` (buffered and streamed via ``stream_chat_completion``) from ``GRAPHRAG_OPENAI_*`` / ``GRAPHRAG_CHAT_MODEL``; optional ``GRAPHRAG_CHAT_TIMEOUT_S`` (seconds) for slow local hardware; buffered ``complete_chat`` supports optional ``response_format`` JSON object mode for callers such as ``analysis_llm_insights``; when ``message.content`` is empty but ``message.reasoning`` is present (some Ollama Cloud models), assistant text is taken from ``reasoning``. Logs each request/response at INFO (host, model, counts, reply length or empty-body diagnostics; HTTP errors log a short body preview).
- **`chat_service.py`**: Orchestrates seeds, subgraph expansion, source excerpts, optional **conversation history** + **carryover summary** for multi-turn prompts, ``approx_input_chars`` / ``context_warn`` heuristics, optional ``context_auto_fork`` when ``GRAPHRAG_CHAT_AUTO_SUMMARY_AT_CHARS`` is met (requires prior turns), and LLM summarization for session fork; logs ``GraphRAG LLM request starting`` / ``reply success`` around each model call. The main system prompt instructs the model to emit **valid GFM tables** (one physical line per row, blank line before tables, fenced code on its own line).
