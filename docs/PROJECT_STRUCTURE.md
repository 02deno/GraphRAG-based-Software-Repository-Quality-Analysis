...
- **`analysis_llm_insights.py`**: Optional one-shot OpenAI-compatible call that turns metrics + ``analysis.txt`` + chart filenames into structured JSON (``graphrag_llm_insights.json``); used by ``web.analysis_results_llm_insights`` and surfaced on ``templates/results_final.html``.
