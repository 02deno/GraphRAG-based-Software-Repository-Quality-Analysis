...
- **Environment file**: optional project-root ``.env`` (copy from ``.env.example``) is loaded on app startup with ``python-dotenv`` before logging and config read; existing shell variables are **not** overridden.
- **GraphRAG env (optional beyond chat)**: ``GRAPHRAG_EMBEDDING_MODEL`` + same OpenAI-compatible base URL for vector seeds (per-run ``graphrag_embedding_cache.npz``); ``GRAPHRAG_NEO4J_URI`` / ``GRAPHRAG_NEO4J_USER`` / ``GRAPHRAG_NEO4J_PASSWORD`` (and optional ``GRAPHRAG_NEO4J_DATABASE``) to run subgraph **expansion** in Neo4j instead of NetworkX; **source index** tuning via ``GRAPHRAG_SOURCE_MAX_FILES``, ``GRAPHRAG_SOURCE_CHUNK_LINES``, ``GRAPHRAG_SOURCE_CHUNK_OVERLAP_LINES``, ``GRAPHRAG_SOURCE_MAX_BYTES_PER_FILE`` (each successful web run writes ``graphrag_run_meta.json`` + ``graphrag_source_chunks.jsonl`` beside ``graph.json`` when the analyzed repo path still exists)
- **GraphRAG workspace (``/graphrag/workspace``)**: send/stream state is **per session**—you can switch to another chat and still press **Send** there while a long reply runs elsewhere. Switching away **aborts the browser’s read** of the previous session’s SSE stream; the server still runs the turn and **writes your question to disk before streaming** so refresh or reopening that session does not show an empty chat. The assistant reply is appended when the model finishes. Live “Steps” lines only render for the session currently on screen. The chat system prompt discourages meta-phrasing (“the provided text…”) and **blends** a short follow-up with the previous user question for retrieval only (seeds and source excerpts), while the visible question stays the latest line.
...
- **Neo4j property graph** (GraphRAG panel): when ``neo4j_driver`` is configured, optional **Load graph preview** fetches ``…/neo4j-property-graph.json`` and renders a capped directed multigraph with **vis-network** (CDN); otherwise a short configuration hint is shown
