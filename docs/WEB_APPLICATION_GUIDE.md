...
- **GraphRAG workspace (``/graphrag/workspace``)**: send/stream state is **per session**—you can switch to another chat and still press **Send** there while a long reply runs elsewhere. Switching away **aborts the browser’s read** of the previous session’s SSE stream (the server may still finish and persist the answer; reopen that session to load the saved messages). Live “Steps” lines only render for the session currently on screen.
